...
| Import/Export | EPPlus (.xlsx), DocNET++ (.docx), iText7 (.pdf) |
...
- `TestHistories`
- `TestResultDetails`
- `LoginAttempts` (theo dõi đăng nhập thất bại, hỗ trợ khóa tài khoản)
...
Script tạo database nằm tại [CreateDB.sql](./CreateDB.sql).
...
Mở SQL Server Management Studio, kết nối tới SQL Server local và chạy file:
...
```text
QuizManagement/appsettings.json
```

Cập nhật `DefaultConnection` theo máy local của bạn.
...
Không nên commit password thật lên repository.

Nếu muốn giữ mật khẩu local mà không đẩy lên Git, copy file mẫu:
...
Sau đó sửa password trong `appsettings.Local.json`. File này đã được ignore bởi Git.
...
2. Tạo **Môn học** → tạo **Bộ đề** → tạo **Câu hỏi** và **Đáp án**.
3. Loại câu hỏi: `1` = một đáp án đúng, `2` = nhiều đáp án đúng.
4. Vào bộ đề → bấm **Bắt đầu Quiz** → cấu hình số câu → làm bài → xem kết quả.
5. Xem lịch sử và thống kê tại `Thống kê`.
...
- Vào `/Admin` để xem tổng quan hệ thống.
- Vào `/Admin/Users` để quản lý danh sách người dùng và thay đổi vai trò.

## Quy Tắc Nghiệp Vụ Đang Áp Dụng

- Người dùng chỉ xem và thao tác dữ liệu của chính mình.
...
- [ ] Seed demo data: ít nhất 2 subjects, 3 decks, 20+ questions, 1 tài khoản Admin.
