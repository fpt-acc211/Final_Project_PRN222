...
| Timer quiz, leaderboard, mentor stats | Hoàn thành |
| Báo cáo câu hỏi sai | Hoàn thành |
| Login attempts audit log | Hoàn thành |
...
| Automated tests | Hoàn thành - 25 test pass |
...
| `QuizManagement.Tests` | Unit/integration tests cho service, repository và controller |
...
`Decks` có thêm `TimeLimitMinutes` để Mentor đặt giới hạn thời gian làm bài. `QuestionReports` lưu báo cáo câu hỏi sai từ người học, còn `LoginAttempts` lưu lịch sử đăng nhập thành công/thất bại để Admin kiểm tra.
...
Question 1 - n QuestionReport
User 1 - n LoginAttempt
```

Script tạo/cập nhật database nằm tại [CreateDB.sql](./CreateDB.sql). Script này có thể chạy trên database mới hoặc database cũ để bổ sung bảng/cột/index/FK còn thiếu mà không cần xóa dữ liệu hiện có. Dữ liệu demo nằm tại [SeedDemoData.sql](./SeedDemoData.sql) và có thể chạy lại nhiều lần.
...
Với database đã tồn tại, vẫn có thể chạy lại `CreateDB.sql`; script sẽ chỉ tạo hoặc bổ sung phần còn thiếu như `TimeLimitMinutes`, `QuestionReports`, `LoginAttempts`, index và foreign key.
...
```bash
dotnet test .\QuizManagementSystem.slnx --no-restore
```

Test project hiện có 25 test bao phủ các luồng chính ở service, repository và controller: quiz, import câu hỏi, báo cáo câu hỏi, login attempts và phân quyền xử lý báo cáo.
...
- **Báo cáo câu hỏi**: Sau khi xem kết quả quiz, user có thể báo cáo câu hỏi sai/chưa rõ/trùng; Mentor/Admin xử lý tại trang báo cáo lỗi.
- **Leaderboard**: Mỗi bộ đề có bảng xếp hạng top điểm cao nhất.
- **Mentor stats**: Mentor xem thống kê học liệu và kết quả học tập trên nội dung mình sở hữu.
...
- Mentor chỉ xử lý báo cáo câu hỏi thuộc học liệu mình sở hữu; Admin xử lý được tất cả báo cáo.
...
- [x] Timer quiz theo `Deck.TimeLimitMinutes`, có kiểm tra hết hạn ở server.
- [x] Leaderboard theo bộ đề và thống kê nội dung cho Mentor.
- [x] Báo cáo câu hỏi sai cho User, Mentor/Admin xử lý.
- [x] Login attempts audit log cho Admin.
...
- [x] Automated tests: 25 test pass với `QuizManagement.Tests`.
