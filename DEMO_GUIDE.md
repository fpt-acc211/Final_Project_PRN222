...
Mở `CreateDB.sql` trong SSMS và chạy toàn bộ script.

Kết quả mong đợi:

- Database `QuizManagementDB` được tạo.
- Các bảng, khóa ngoại, index và constraint được tạo đầy đủ.
- Chạy lại script không xóa dữ liệu nếu schema đang đúng phiên bản.

Nếu đang dùng database từ phiên bản cũ trước ngày 22/07/2026, không chạy lại `CreateDB.sql`; chạy `UpgradeDB_20260722.sql` một lần.

### 3.3. Nạp dữ liệu demo

1. Mở `SeedDemoData.sql` trong SSMS.
2. Tạm dán đoạn sau vào đầu cửa sổ query, trước nội dung hiện có:
...
3. Chạy toàn bộ nội dung trong cùng cửa sổ query.
4. Không cần lưu thay đổi vừa dán vào file.

Kết quả cuối script phải hiển thị:

- Ba tài khoản `Admin`, `Mentor`, `User`.
- 2 môn học, 3 bộ đề và 21 câu hỏi.
- Dữ liệu mẫu về lượt làm quiz, báo cáo câu hỏi và lịch sử đăng nhập.

Nếu gặp lỗi `51019`, lệnh opt-in và seed đã không chạy trong cùng SQL session.
...
Chạy lệnh `sp_set_session_context` và toàn bộ seed trong cùng một query window/session.
...
Để đưa các tài khoản seed, nội dung, lịch sử, báo cáo và login attempt về trạng thái mẫu, chạy lại `SeedDemoData.sql` với opt-in như phần 3.3.

Lưu ý: seed không xóa tài khoản tạm không thuộc seed, ví dụ `live.demo@quiz.local`. Có thể vô hiệu hóa tài khoản này trong trang Admin.

Nếu cần làm sạch hoàn toàn database demo, có thể xóa và tạo lại database. Thao tác sau **xóa toàn bộ dữ liệu**:
...
Sau đó chạy lại `CreateDB.sql` và `SeedDemoData.sql`.

## 9. Checklist ngay trước khi trình bày

- [ ] SQL Server đang chạy.
- [ ] `QuizManagementDB` đúng schema hiện tại.
